...
def apiRequest(reqargs):
    ...
    try:
        ...
        raise KeyException("%s: %s"%(errorcode,errormess))
    ...
    if statuscode == 200:
        ...
        raise networkException(statuscode)
...
class Randomorg:
    def __init__(self, apiKey):
        ...
        try:
            errorcode = response["error"]["code"]
            errormess = response["error"]["message"]
            raise ApiException("%s: %s"%(errorcode,errormess))
        ...
        if response["result"]["status"] == "stopped":
            raise ApiException("Api key is stopped.")
       
        if response["result"]["requestsLeft"] == 0:
            raise ApiException("Requests limit reached.")
